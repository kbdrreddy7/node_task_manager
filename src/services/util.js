...
const parseQueryString=(queryString)=>{
    ...
    for (let field in where)
    {
        ...
        /* 
        // if field value is an object...then that is operation based filter 
            e.g name:{
                like :'%ram%'
            }
        */ 
...
const convertToToken = (payload) => jwt.sign(payload, process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPIRY_TIME});

const decodeToken=(token)=>jwt.verify(token, process.env.JWT_SECRET);

const hashPassword=(password)=> bcrypt.hashSync(password, 10)
//console.log("--------------------------------password hashing \n",hashPassword("Ilove*ta2"))
